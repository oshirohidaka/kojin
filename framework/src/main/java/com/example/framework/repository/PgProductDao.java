...
public class PgProductDao implements ProductDao {
    ...
    public List<ProductRecord> findByKeyword(String name) {
        ...
        return jdbcTemplate.query("SELECT product_id,products.name,price,categories.name AS categories_name FROM products INNER JOIN categories ON products.category_id = categories.id WHERE products.name LIKE '%"+name+"%' ORDER BY product_id;", param, new DataClassRowMapper<>(ProductRecord.class));
    ...
    public int insert(String product_id,String name,int price,int category_id) {
        ...
        return jdbcTemplate.update("INSERT INTO products(product_id,name,price,category_id) VALUES(:product_id,:name,:price,:category_id)", param);
    }
